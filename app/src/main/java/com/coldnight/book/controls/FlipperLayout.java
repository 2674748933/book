        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                ...
                if (mDirection == MOVE_NO_RESULT) {
                    if (mListener.whetherHasNextPage(index) && distance > 0) { //左滑
...
//                }
                ...
                if (mMode == MODE_NONE
                        && ((mDirection == MOVE_TO_LEFT && mListener.whetherHasNextPage(index)) || (mDirection == MOVE_TO_RIGHT
                        && index > 1))) {
                    mMode = MODE_MOVE;
                }
                ...
                if (mDirection != MOVE_NO_RESULT) {
                    ...
                    if (mMode == MODE_MOVE) {
                        ...
                        if (mDirection == MOVE_TO_LEFT && scrollX != 0 && mListener.whetherHasNextPage(index)) {
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            Log.d(TAG, "computeScroll: 滚动 当前页" + index);
            if (mTouchResult == MOVE_TO_LEFT) { //下一页
                ...
                if (index == 1) {
                    ...
                    currentTopView = mListener.createView(mTouchResult, index-1);
                    currentTopView.scrollTo(-screenWidth, 0);
                    Log.d(TAG, "当前页: " + index + "创建上一页");
                }
                addView(currentTopView);
            }
    ...
    public interface TouchListener {
         ...
         * @return
         */
        boolean whetherHasNextPage(int index);
    }
